...
% 平台与容器参数
L_container = 10.0;    % 容器边长 (m)
L_platform  = 0.5;    % 平台边长 (m)
p_center = [3; -5; 6]; % 平台质心位置

% 计算平台挂载点相对于平台质心的向量 (8个角)
r_platform = [ L_platform/2,  L_platform/2,  L_platform/2,  L_platform/2, -L_platform/2, -L_platform/2, -L_platform/2, -L_platform/2;
               L_platform/2,  L_platform/2, -L_platform/2, -L_platform/2,  L_platform/2,  L_platform/2, -L_platform/2, -L_platform/2;
               L_platform/2, -L_platform/2,  L_platform/2, -L_platform/2,  L_platform/2, -L_platform/2,  L_platform/2, -L_platform/2];

% 平台挂载点位置
p = repmat(p_center,1,8) + r_platform;

% 定义容器锚点（正方体容器的八个角）
B = [ L_container/2,  L_container/2,  L_container/2,  L_container/2, -L_container/2, -L_container/2, -L_container/2, -L_container/2;
      L_container/2,  L_container/2, -L_container/2, -L_container/2,  L_container/2,  L_container/2, -L_container/2, -L_container/2;
      L_container/2, -L_container/2,  L_container/2, -L_container/2,  L_container/2, -L_container/2,  L_container/2, -L_container/2];
