function [J_inv, singular_vals, cond_number] = analyzeJacobianNonDim(x0,  delta)
...
%     2) 非量纲化采用 x_nd = [x/L; angles/L_theta]。
    ...
    L2 = 0.09;
    LC = 0.6;
    L_theta = sqrt(2*(L1^2)+2*(L2^2));
    % 2) 构造非量纲化缩放矩阵 S_inv = diag(L, L, L, L_theta, L_theta, L_theta)
    S_inv = diag([L1/LC, L1/LC, L2/LC, L_theta/LC, L_theta/LC, L_theta/LC]);
